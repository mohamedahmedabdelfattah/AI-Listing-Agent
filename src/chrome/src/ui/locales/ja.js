...
export default {
  ...
  'ob.btn.skip': '今はスキップ',
  'ob.btn.start': '開始',
  'ob.cloud.body': 'WebBrain Cloud は1日あたりの無料割り当てで準備できています。選択した Cloud 会話は、WebBrain の改善を支援がデフォルトでオンになっている間、WebBrain の改善のために保存および使用される可能性があります。設定 → 一般でオフにできます。',
  'ob.cloud.change': '変更',
  'ob.cloud.using': 'WebBrain Cloud を使用しています。',
  ...
  'st.vision.local.desc': 'LiquidAI/LFM2.5-VL-450M-ONNXをGPU上で実行します。スクリーンショットはこのデバイス内に保持されます。初回使用時に、Hugging Faceから約770 MBをバックグラウンドでダウンロードし、ローカルにキャッシュします。ダウンロード中はタブの切り替えや設定画面を閉じることができますが、Chromeは開いたままにしてください。',
  ...
  'st.vision.local.testing': 'ローカルビジョンを読み込んでテストしています… 初回使用時には約770 MBをバックグラウンドでダウンロードします。タブの切り替えや設定画面を閉じることができますが、Chromeは開いたままにしてください。',
  ...
  "sp.slash.watch": "条件を満たすまでこのページを監視",
  "sp.slash.watch_keep": "異なる一致の後も監視を継続",
  "sp.slash.watch_secs": "確認間隔を秒単位で設定",
  "sp.slash.watch_long": "長い監視アラートを使用",
  "sp.slash.watch_short": "短い監視アラートを使用",
  "sp.watch.created": "このページを {seconds} 秒ごとに監視しています。",
  "sp.watch.exists": "このページはすでに同じ条件で監視されています。",
  "sp.watch.error": "監視を作成できませんでした: {error}",
  ...
  "st.captcha.saved": "保存しました。CapSolver が有効になりました。",
  ...
  'sp.scheduled.watch_keep': '一致のたびに監視を継続',
  ...
  'st.display.cloud_bridge.status_disabled': 'ブリッジは無効です',
  'st.display.cloud_bridge.status_connecting': '接続中…',
  'st.display.cloud_bridge.status_connected': '接続済み',
  'st.display.cloud_bridge.status_reconnecting': '再接続中（{attempt} 回目）…',
  'st.display.cloud_bridge.status_error': '接続エラー: {error}',
  'st.display.cloud_bridge.status_unreachable': '{url} に接続できません。対応するローカルブリッジサービスを起動してください。再試行中…',
  ...
  'st.display.plan_before_act.desc': "Act と Dev はツール実行前に必ず構造化された意図確認を行います。試行（デフォルト）は完全な計画も作成しますが、短いフォローアップでは直近に承認された計画を再利用できます。厳格は毎ターン完全な計画を作成します。意図または計画が無効な場合、試行は1回再試行してから警告を表示し、Act モードで続行します。厳格はツール実行前に停止します。",
  ...
  'sp.plan.step_placeholder': 'ステップの操作',
  ...
  "sp.slash.delete_workflow": "IDで保存済みワークフローを削除",
  "sp.workflows.title_html": "<strong>保存済みワークフロー</strong>",
  "sp.workflows.empty": "保存済みワークフローはありません。トレースを有効にしてタスクを完了し、/workflow --save <name> を使用してください。",
  "sp.workflows.item": "{id}  {name}  ({steps} ステップ、{parameters} パラメーター)",
  "sp.workflows.meta": "{id} · {steps} ステップ · {parameters} パラメーター",
  "sp.workflows.delete_confirm": "削除を確認",
  "sp.workflows.rename": "名前を変更",
  "sp.workflows.saved": "ワークフロー「{name}」を保存しました。",
  "sp.workflows.deleted": "ワークフロー「{name}」を削除しました。",
  "sp.workflows.error": "ワークフローエラー: {msg}",
  "sp.teach.started": "「{name}」を学習中です。操作を完了してから /teach --end を使用してください。",
  "sp.teach.status": "「{name}」を学習中：{count} 件の操作を記録しました。",
  "sp.teach.inactive": "ティーチャーモードは有効ではありません。",
  "sp.teach.saved": "ワークフロー「{name}」を {count} ステップで学習しました。",
  "sp.teach.already_active": "このタブではティーチャーモードがすでに有効です。",
  "sp.teach.agent_running": "ティーチャーモードを開始する前に実行中のエージェントを停止してください。",
  "sp.teach.capture_unavailable": "このページはティーチャーモードで記録できません。通常の http(s) ページを開くか再読み込みして、もう一度お試しください。",
  "sp.workflows.reason.no_trace": "この会話に成功した記録済み実行がありません。トレースを有効にしてタスクを完了し、保存してください。",
  ...
  "st.imageBudget.heading": "画像予算",
  "st.imageBudget.desc": "スクリーンショットのサイズと、エージェントが 1 ターンあたりビジョン用に撮影する回数を制御します。詳細度と寸法を下げると小型エンドポイントでのコストと遅延が減り、上げると忠実度が保たれます。既定値は以前の動作と同じです。",
  "st.imageBudget.detail.label": "画像の詳細度",
  "st.imageBudget.detail.desc": "モデルに送るビジョン画像の詳細度。「high」はより多くの詳細を保持（トークンとコスト増）；「low」は小さく安価な画像；「auto」はプロバイダ任せ。プロバイダ依存です。",
  "st.imageBudget.detail.high": "高（詳細多・コスト高）",
  "st.imageBudget.detail.low": "低（安価・詳細少）",
  "st.imageBudget.detail.auto": "自動（プロバイダ既定）",
  "st.imageBudget.maxPerTurn.label": "1 ターンあたりの最大スクリーンショット数",
  "st.imageBudget.maxPerTurn.desc": "エージェントが 1 ターンでビジョン用に自動撮影できる回数（0 = 無制限）。低い値はコストを下げます。予算を使い切ると、そのターンの以降の自動撮影はスキップされます。",
  "st.imageBudget.maxPerTurn.unlimited": "無制限",
  ...
  "st.sync.title": "暗号化されたクラウド同期",
  "st.sync.lede_html": "<strong>WebBrain Cloud メンバーのみ</strong> デバイス間でメモリ、プロファイルの自動入力、API キー プロバイダーの設定を同期します。アップロード前に、すべてが同期パスワードで暗号化されます。 WebBrain はそれを読み取ることも回復することもできません。チャット履歴と OAuth サインインは同期されません。",
  "st.sync.email.label": "WebBrain クラウドメール",
  "st.sync.password.label": "暗号化用の同期パスワード (12 文字以上)",
  "st.sync.confirm.label": "同期パスワードの確認",
  "st.sync.action.auth": "サインインリンクを送信する",
  "st.sync.action.enable": "暗号化された同期をオンにする",
  "st.sync.action.unlock": "暗号化された同期のロックを解除する",
  "st.sync.action.now": "今すぐ同期する",
  "st.sync.action.lock": "ロック",
  "st.sync.action.change_password": "同期パスワードを変更する",
  "st.sync.action.disable": "このデバイスの同期をオフにする",
  "st.sync.action.reset": "クラウドコピーをこのデバイスに置き換えます",
  "st.sync.status.syncing": "暗号化された同期を更新中です…",
  "st.sync.status.offline": "暗号化された同期は接続を待機しています。",
  "st.sync.status.subscription": "暗号化同期には WebBrain Cloud メンバーシップが必要です。",
  "st.sync.status.error": "暗号化された同期には注意が必要です。",
  "st.sync.status.auth_required": "暗号化された同期を使用するには、WebBrain Cloud メールでサインインします。",
  "st.sync.status.password_required": "サインインしています。同期パスワードを選択して、暗号化された同期を有効にします。",
  "st.sync.status.unlocked": "このデバイスでは暗号化された同期がオンになっています。",
  "st.sync.status.locked": "暗号化された同期はロックされています。同期パスワードを入力して、このデバイスのロックを解除します。",
  "st.sync.result.updated": "暗号化された同期が更新されました。",
  "st.sync.result.current": "暗号化された同期は最新です。",
  "st.sync.pending.syncing": "暗号化されたクラウド コピーを同期しています…",
  "st.sync.pending.syncing_short": "同期中…",
  "st.sync.error.generic": "暗号化された同期が失敗しました: {error}",
  "st.sync.error.unknown": "不明なエラー",
  "st.sync.validation.email_required": "WebBrain Cloud の請求先メールアドレスを入力します。",
  "st.sync.validation.password_length": "少なくとも 12 文字の同期パスワードを使用してください。",
  "st.sync.validation.confirm_required": "同期パスワードを確認します。",
  "st.sync.validation.password_mismatch": "同期パスワードが一致しません。",
  "st.sync.prompt.new_password": "新しい同期パスワード",
  "st.sync.prompt.password": "{label} (12 文字以上):",
  "st.sync.prompt.confirm_password": "同期パスワードを確認します:",
  "st.sync.prompt.current_password": "現在の同期パスワード:",
  "st.sync.prompt.replacement_password": "置換クラウドコピーの同期パスワード",
  "st.sync.auth.check_email": "メールを確認し、WebBrain Cloud サインイン リンクを承認して、ここに戻ってください。",
  "st.sync.auth.success": "クラウド同期が認証されました。パスワードを設定し、同期を有効にします。",
  "st.sync.confirm.disable": "このデバイスの暗号化された同期をオフにしますか?ローカルデータは残ります。",
